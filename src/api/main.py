...
class CustomerAggregator(BaseEstimator, TransformerMixin):
    """
    Custom transformer to aggregate transaction data to a customer level.
    Calculates Recency, Frequency, Amount statistics, and most frequent categorical values.
    """
...
class Transaction(BaseModel):
    """
    Defines the structure of a single transaction data point expected by the API.
    Includes type hints, default values, examples, and descriptions for OpenAPI.
    """
    CustomerId: str = Field(..., example="C123", description="Unique identifier for the customer.")
    TransactionId: str = Field(..., example="T1001", description="Unique identifier for the transaction.")
    TransactionStartTime: datetime = Field(..., example="2025-06-15T14:00:00", description="Start time of the transaction (ISO 8601 format).")
    CountryCode: str = Field(..., example="ET", description="ISO 3166-1 alpha-2 country code of the transaction.")
    CurrencyCode: str = Field(..., example="ETB", description="ISO 4217 currency code of the transaction.")
    ChannelId: int = Field(..., example=1, description="Identifier for the transaction channel (e.g., 1 for mobile, 2 for web).")
    Amount: float = Field(..., example=200.5, gt=0, description="Transaction amount (must be positive).")
    Value: float = Field(..., example=1000.0, gt=0, description="Transaction value (must be positive).")
    AccountId: str = Field(..., example="A55", description="Account identifier involved in the transaction.")
...
EXTRA_VARS = ['Frequency', 'AvgTransactionHour', 'MostFrequentDayOfWeek', 'Recency', 'AccountFrequency']
...
async def predict(transactions: List[Transaction]):
    ...
    if not all([loaded_model, feature_pipeline, expected_final_columns]):
        ...
        raise HTTPException(status_code=500, detail="Model or preprocessing pipeline not properly loaded.")
    ...
    try:
        ...
        print(f"DEBUG: customer_features_df columns: {customer_features_df.columns.tolist()}")
        ...
        if 'Amount_std' in customer_features_df.columns:
            print(f"DEBUG: Amount_std after aggregation (first few values): {customer_features_df['Amount_std'].head().tolist()}")
        ...
        # This handles cases where a column might be missing if the input df is very sparse
        transform_input = customer_features_df.reindex(columns=cols_for_pipeline, fill_value=np.nan)

        print(f"DEBUG: transform_input columns for pipeline: {transform_input.columns.tolist()}")
        ...
        transformed_df = pd.DataFrame(transformed_array, columns=feature_pipeline.get_feature_names_out())
        print(f"DEBUG: transformed_df columns after pipeline: {transformed_df.columns.tolist()}")
        ...
        extra_vars_df = customer_features_df[EXTRA_VARS].reset_index(drop=True).fillna(0)
        ...
        # Step 5: Ensure final column alignment with the model's expected features
        # This is critical for model prediction stability
        missing_cols = set(expected_final_columns) - set(X_final.columns)
        for col in missing_cols:
            X_final[col] = 0.0 # Add missing columns with a default value (e.g., 0)
        # Ensure columns are in the exact order expected by the model
        X_final = X_final[expected_final_columns]
        print(f"DEBUG: X_final columns before prediction (final order): {X_final.columns.tolist()}")
        ...
        for i, cid in enumerate(customer_features_df["CustomerId"]):
            results.append({
                "CustomerId": cid,
                "risk_probability": float(risk_probs[i]) # Convert numpy float to Python float
            })
